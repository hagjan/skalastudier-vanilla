...
class IntervalTableComponent extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.#intervals = intervals ? intervals.split(',') : []
  ...
  update() {
    ...
    const filtered = this.#intervals.length > 0 ?
      this.#interval_list.filter(item => this.#intervals.includes(item.fraction)) : this.#interval_list
    filtered.sort((a, b) => a.value < b.value ? 1 : -1).forEach(item => {
